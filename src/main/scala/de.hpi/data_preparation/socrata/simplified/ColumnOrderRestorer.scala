...
class ColumnOrderRestorer extends StrictLogging{
  ...
  def restoreInDataset(id:String, version:LocalDate,runCorrection:Boolean = false) = {
    ...
    try {
      ds = Socrata_IOService.loadSimplifiedRelationalDataset(socrata.DatasetInstance(id, version))
    } catch {
      case e: Throwable => {
    ...
    if(ds!=null) {
      ...
      ds.toJsonFile(new File(Socrata_IOService.getSimplifiedDatasetFile(socrata.DatasetInstance(id, version)) + "_repaired.json?"))
    }
  }
